...
public class QuestDataManager {
    ...
    private final SeasonDataManager seasonDataManager = new SeasonDataManager();

    private final Collection<Team> formerGrounds = new ArrayList<>();

    @PostConstruct
    void initialise() throws IOException {
        initialiseDivisions();
        initialiseFormerGrounds("/former-league.dat");
        initialiseFormerGrounds("/lost-league-grounds.dat");
    }

    private void initialiseFormerGrounds(String fileLocation) throws IOException {
        InputStream stream = QuestDataManager.class.getResourceAsStream(fileLocation);
    ...
    private void initialiseDivisions() throws IOException {
    ...
    private Division processDivision(String[] parts) {
        Division d;
    ...
    private Team processTeam(String[] parts) {
        Team t;// do something with id
        String id = parts[1];
        String name = parts[2];
        t = new Team(name);
        return t;
    ...
    private void processData(Team t, String[] parts) {
    ...
    public Collection<Season> getSeasons() {
        return seasonDataManager.getSeasons(getVisits(), formerGrounds);
    }
